...
namespace YoutubeBlog.Web.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Detail(Guid id)
        {
            ...
            var articeVisitors = await unitOfWork.GetRepository<ArticleVisitor>().GetAllAsync(null, x => x.Visitor, y => y.Article); //coka cok tablolardakı bılgılerı aldık
            ...
            if (articeVisitors.Any(x => x.VisitorId == addArticleVisitors.VisitorId && x.ArticleId == addArticleVisitors.ArticleId)) //aynı visitorler varsa işlem yapmadan sayfama dönebiliriz
                return View(result);
            else
            {
                await unitOfWork.GetRepository<ArticleVisitor>().AddAsync(addArticleVisitors);
                article.ViewCount += 1; //ziyaretci sayısını bir artir
                await unitOfWork.GetRepository<Article>().UpdateAsync(article); //sonrasında article deki ziyaretci 1 arttıgı için sayıyı güncelliyoruz
...
//makalelerin detaylarına gitmemizi sağlayan yazının devamını okuya bastıgımızda buralara yönledirecek sayfamızı olusturuyoruz.